...
overview: 基于需求说明与租房 API 接口文档，对「智找安居·马年省心办」租房 AI Agent 挑战赛进行赛题解析，并给出架构设计、接口映射、评分要点。
todos: []
isProject: false
---
...
2. **租房 API 使用**
  - 所有 `/api/houses/`* 请求必须带 `**X-User-ID`**（默认为f00952481），否则 400。
...
记录 Agent 执行过程中的 API 调用信息，便于迭代分析执行问题。日志信息对重复内容进行精简（如LLM的prompt）。
...
接口语义可从 [需求说明.md](需求说明.md) 与 [租房API接口.json](租房API接口.json) 完全确定，实现时按「约定」调用即可。
